...
public class TSPSolverHalf {
    ...
    public int[] solve() {
        ...
        int[] currentTour = solveHalfTSP(cities, (int) Math.ceil(numCities / 2.0));
        int currentTourLength = calculateTourLength(currentTour);

        while (twoOptSwap(currentTour, currentTourLength)) {
    ...
    public static int[] solveHalfTSP(List<City> cities, int selectedCities) {
    ...
    private boolean twoOptSwap(int[] tour, int tourLength) {
